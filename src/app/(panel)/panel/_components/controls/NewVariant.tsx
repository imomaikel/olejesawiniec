...
const NewVariant = ({ productId, refetchProduct }: TNewVariant) => {
	...
	const { mutate: createVariant, isLoading } =
		trpc.panel.createVariant.useMutation({
		...
		<>
			...
			<Form {...form}>
				<form
					...
					className='flex items-center flex-col xl:flex-row lg:space-y-4'
				>
					...
					<Button
						...
						size='xl'
						disabled={isLoading}
						className='rounded-full w-full mt-4 mlg:mt-0 lg:w-auto'
					>
						Dodaj
					</Button>
				</form>
