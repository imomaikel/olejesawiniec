...
const ProductManager = ({ product, refetch }: TProductManager) => {
  ...
  const { mutate: deleteProduct, isLoading: isDeleting } = trpc.panel.deleteProduct.useMutation({
    onSuccess: () => {
    ...
    <div>
      ...
      <div>
        <div className="flex flex-col md:flex-row items-center space-x-0 md:space-x-2 space-y-2 md:space-y-2">
          <h3 className="font-bold text-lg md:mt-1.5">Aktualny stan produktu</h3>
        ...
        <div className="mt-2 space-x-0 md:space-x-2 space-y-2 md:space-y-0 flex flex-col md:flex-row">
          {productStatus === 'enabled' ? (
            ...
            <ActionButton onClick={() => changeState({ newState: true, productId: product.id })} disabled={isLoading}>
              Włącz produkt
            </ActionButton>
          )}
      ...
      <AlertDialog open={isOpen} onOpenChange={setIsOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Usunięcie produktu jest nieodwracalne. Czy na pewno chcesz to zrobić?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            {!isDeleting && (
