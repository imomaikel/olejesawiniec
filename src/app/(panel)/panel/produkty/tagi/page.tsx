...
const TagsPage = () => {
	...
	const { mutate: removeTag, isLoading: isRemoving } =
		trpc.panel.removeTag.useMutation({
			onSuccess: (message) => {
					...
					{tagList
						? tagList
								...
								.map(({ label }) => (
									...
									>
										...
										<span className={cn(isRemoving && 'invisible')}>
											{label}
										</span>
									</Badge>
								))
						: [...Array.from(Array(12).keys())].map((index) => {
								...
								return (
									<Skeleton
										key={`skeleton${index}`}
										style={{ width: `${randomWidth}px`, height: '22px' }}
									/>
								);
						  })}
				</div>
