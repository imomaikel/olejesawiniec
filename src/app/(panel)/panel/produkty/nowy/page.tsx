...
const NewProductPage = () => {
  ...
  const { mutate: createProduct, isLoading: isCreating } = trpc.panel.createProduct.useMutation({
  ...
  const { mutate: createCategory, isLoading: isCategoryCreating } = trpc.panel.createCategory.useMutation({
    onSuccess: (label) => {
  ...
  const handleSubmit = (e: KeyboardEvent) => {
    if (e.key === 'Enter' && !isCreating) createProduct({ productName, categoryLabel: selectedCategory });
  };
    ...
    <div className="flex flex-col space-y-4">
      ...
      <div className="flex flex-col space-y-6">
        ...
        <div>
          ...
          <div className="flex flex-col space-y-2">
            <Select onValueChange={(categoryName) => setSelectedCategory(categoryName)}>
              <SelectTrigger>
        ...
        <div>
          ...
          <div className="flex space-x-1">
            ...
            />
            <Button size="icon" onClick={() => createCategory({ label: newCategory })}>
              <FaPlus className="w-4 h-4" />
            </Button>
          </div>
        </div>
        <Button disabled={isCreating} onClick={() => createProduct({ productName, categoryLabel: selectedCategory })}>
          Utwórz
        </Button>
      </div>
    </div>
